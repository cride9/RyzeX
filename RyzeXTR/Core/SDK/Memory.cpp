...
void* MEM::GetExportAddress(const void* hModuleBase, const char* szProcedureName)
{
	...
	{
		//L_PRINT(LOG_ERROR) << ("module has no exports: \"") << GetModuleBaseFileName(hModuleBase) << ("\"");
		return nullptr;
	}
	...
	//L_PRINT(LOG_ERROR) << ("export not found: \"") << szProcedureName << ("\"");
	return nullptr;
}
...
void* MEM::GetImportAddress(const void* hModuleBase, const char* szProcedureName)
{
	...
	{
		//L_PRINT(LOG_ERROR) << ("module has no imports: \"") << GetModuleBaseFileName(hModuleBase) << ("\"");
		return nullptr;
	}
	...
	//L_PRINT(LOG_ERROR) << ("import not found: \"") << szProcedureName << ("\"");
	return nullptr;
}
...
bool MEM::GetSectionInfo(const void* hModuleBase, const char* szSectionName, std::uint8_t** ppSectionStart, std::size_t* pnSectionSize)
{
	...
	//L_PRINT(LOG_ERROR) << ("code section not found: \"") << szSectionName << ("\"");
	return false;
}
#pragma endregion
...
std::uint8_t* MEM::FindPattern(const wchar_t* wszModuleName, const char* szBytePattern, const char* szByteMask)
{
	...
	if (!vecFoundOccurrences.empty())
	{
		...
		if (vecFoundOccurrences.size() > 1U)
		{
			...
			L_PRINT(LOG_WARNING) << ("found more than one occurrence with \"") << szPattern << ("\" pattern, consider updating it!");
...
RTTITypeDescriptor_t* MEM::FindClassTypeDescriptor(const void* hModuleHandle, const char* szClassName)
{
	...
	if (GetSectionInfo(hModuleHandle, (".data"), &pDataStart, &nDataSize))
	{
		...
		CRT::StringCat(CRT::StringCat(CRT::StringCopy(szTypeDescriptorName, (".?AV")), szClassName), ("@@"));
	...
	//L_PRINT(LOG_ERROR) << ("class type descriptor not found: \"") << szClassName << ("\"");
	return nullptr;
}
...
std::uint8_t* MEM::FindVTable(const wchar_t* wszModuleName, const char* szVTableName)
{
	...
	//L_PRINT(LOG_ERROR) << ("virtual table pointer not found: \"") << szVTableName << ("\"");
	return nullptr;
}
#pragma endregion
