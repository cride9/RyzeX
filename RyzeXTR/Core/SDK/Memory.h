...
namespace ROP
{
	...
	__declspec(naked) T __fastcall InvokeFastCall(std::uintptr_t ecx, std::uintptr_t edx, std::uintptr_t uFunctionAddress, SpoofContext_t* pContext, std::uintptr_t uGadgetAddress, Args_t... argList)
	{
		__asm
		{
	...
	template <typename T, typename... Args_t>
	__declspec(naked) T __cdecl InvokeCdecl(std::uintptr_t uFunctionAddress, SpoofContext_t* pContext, std::uintptr_t uGadgetAddress, Args_t... argList)
	{
		__asm
		{
			mov eax, [esp + 8] // load a pointer to context
...
namespace MEM
{
	...
	[[nodiscard]] __forceinline std::uint8_t* GetAbsoluteAddress(std::uint8_t* pRelativeAddress)
	{
	...
	inline void* (__cdecl* fnRTDynamicCast)(void* pPolymorphObject, int nVFDelta, void* pSourceType, void* pTargetType, bool bIsReference) = nullptr;
