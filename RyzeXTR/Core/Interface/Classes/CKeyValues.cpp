...
CKeyValues::CKeyValues(const char* szKeyName, void* pUnknown1, HKeySymbol hCaseInsensitiveKeyName)
{
	...
	static CKeyValuesConstructorFn oConstructor = reinterpret_cast<CKeyValuesConstructorFn>(util::FindSignature("client.dll", "55 8B EC 56 8B F1 33 C0 8B 4D 0C 81")); // @xref: client.dll -> "OldParticleSystem_Destroy"
...
CKeyValues::~CKeyValues()
{
	...
	static CKeyValuesDestructorFn oDestructor = reinterpret_cast<CKeyValuesDestructorFn>(util::FindSignature("client.dll", "56 8B F1 E8 ? ? ? ? 8B 4E 14"));
	oDestructor(this, 1);
}
...
{
	static auto oFromString = util::FindSignature("client.dll", "55 8B EC 81 EC ? ? ? ? 85 D2 53"); // @xref: "#empty#", "#int#"
...
void CKeyValues::LoadFromBuffer(char const* szResourceName, const char* szBuffer, void* pFileSystem, const char* szPathID, GetSymbolProcFn pfnEvaluateSymbolProc)
{
	...
	static auto oLoadFromBuffer = reinterpret_cast<LoadFromBufferFn>(util::FindSignature("client.dll", "55 8B EC 83 E4 F8 83 EC 34 53 8B 5D 0C 89")); // @xref: "KeyValues::LoadFromBuffer(%s%s%s): Begin"
...
bool CKeyValues::LoadFromFile(void* pFileSystem, const char* szResourceName, const char* szPathID, GetSymbolProcFn pfnEvaluateSymbolProc)
{
	...
	static auto oLoadFromFile = reinterpret_cast<LoadFromFileFn>(util::FindSignature("client.dll", "55 8B EC 83 E4 F8 83 EC 14 53 56 8B 75 08 57 FF")); // @xref: "rb"
...
CKeyValues* CKeyValues::FindKey(const char* szKeyName, const bool bCreate)
{
	...
	static auto oFindKey = reinterpret_cast<FindKeyFn>(util::FindSignature("client.dll", "55 8B EC 83 EC 1C 53 8B D9 85 DB"));
...
const char* CKeyValues::GetString(const char* szKeyName, const char* szDefaultValue)
{
	...
	static auto oGetString = reinterpret_cast<GetStringFn>(util::FindSignature("client.dll", "55 8B EC 83 E4 C0 81 EC ? ? ? ? 53 8B 5D 08"));
...
void CKeyValues::SetString(const char* szKeyName, const char* szStringValue)
{
	...
	static auto oSetString = reinterpret_cast<SetStringFn>(util::FindSignature("client.dll", "55 8B EC A1 ? ? ? ? 53 56 57 8B F9 8B 08 8B 01"));
