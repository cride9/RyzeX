...
public:
	...
	{
		util::CallVFunc<void>( this, 11, bEnable );
	}

	bool IsButtonDown( EButtonCode buttonCode )
	{
		return util::CallVFunc<bool>( this, 15, buttonCode );
	}

	void ResetInputState( )
	{
		util::CallVFunc<void>( this, 39 );
	}

	const char* ButtonCodeToString( EButtonCode buttonCode )
	{
		return util::CallVFunc<const char*>( this, 40, buttonCode );
	}

	EButtonCode VirtualKeyToButtonCode( int iVirtualKey )
	{
		return util::CallVFunc<EButtonCode>( this, 45, iVirtualKey );
	}

	void GetCursorPosition( int* pX, int* pY )
	{
		util::CallVFunc<void>( this, 56, pX, pY );
	}
};