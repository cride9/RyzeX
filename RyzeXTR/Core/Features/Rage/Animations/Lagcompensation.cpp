...
void Lagcompensation::FrameStageNotify( ) {
	...
	for ( int i = 1; i <= i::GlobalVars->nMaxClients; i++ ) {
		...
		{
			pPrevious = std::make_unique<LagRecord_t>( pLog->pRecord.front( ) );
			pPrevious.get( )->bRestoreData = true;
		}
		...
		if ( pPrevious.get( )->flSimulationTime > pRecordUnique.get( )->flSimulationTime )
		{
			...
			if ( &pPrevious )
				flOldSimulationTime = pPrevious.get( )->flSimulationTime;

			pLog->flExploitTime = flOldSimulationTime;
		...
		if ( pRecordUnique.get( )->flSimulationTime <= pLog->flExploitTime )
		{
			pRecordUnique.get( )->bValid = false;
			pRecordUnique.get( )->bBreakingLagcompensation = true;
		}

		if ( pPrevious.get( )->bRestoreData )
		{
			if ( ( pRecordUnique.get( )->vecOrigin - pPrevious.get( )->vecOrigin ).Length2DSqr( ) > 4096.f )
			{
				pRecordUnique.get( )->bBreakingLagcompensation = true;
				pLog->InvalidateRecords( );
			}
		...
		if ( pRecordUnique.get( )->flSimulationTime < pPrevious.get( )->flSimulationTime ) {

			pRecordUnique.get( )->bValid = false;
			pRecordUnique.get( )->bBreakingLagcompensation = true;

			if ( pPrevious.get( )->bRestoreData )
				pRecordUnique.get( )->iChoked = TIME_TO_TICKS( pRecordUnique.get( )->flOldSimulationTime - pPrevious.get( )->flOldSimulationTime );
			else
				pRecordUnique.get( )->iChoked = pPrevious.get( )->iChoked;
		}
		else {
			if ( pPrevious.get( )->bRestoreData )
				pRecordUnique.get( )->iChoked = TIME_TO_TICKS( pRecordUnique.get( )->flSimulationTime - pPrevious.get( )->flSimulationTime );
			else
				pRecordUnique.get( )->iChoked = TIME_TO_TICKS( pRecordUnique.get( )->flSimulationTime - pRecordUnique.get( )->flOldSimulationTime );
		}

		if ( pLog->bLeftDormancy )
			pRecordUnique.get( )->bFirstAfterDormant = true;

		pLog->pRecord.push_front( *pRecordUnique.get( ) );
...
void Lagcompensation::StartLagcompensation( CBaseEntity* pLocal ) {
	...
	for ( size_t i = 0; i < i::GlobalVars->nMaxClients; i++ ) {
		...
		std::shared_ptr<Lagcompensation::LagRecord_t> pData = std::make_shared<Lagcompensation::LagRecord_t>( );

		pData.get( )->vecAbsAngles = pEntity->GetAbsAngles( );
		pData.get( )->vecAbsOrigin = pEntity->GetAbsOrigin( );
		pData.get( )->vecEyeAngles = pEntity->GetEyeAngles( );
		pData.get( )->vecOrigin = pEntity->GetVecOrigin( );
		pData.get( )->flSimulationTime = pEntity->GetSimulationTime( );
		pData.get( )->iFlags = pEntity->GetFlags( );
		pData.get( )->vecMins = pEntity->GetCollideable( )->OBBMins( );
		pData.get( )->vecMaxs = pEntity->GetCollideable( )->OBBMaxs( );
		pData.get( )->flCollisionChangeTime = pEntity->GetCollisionChangeTime( );
		pData.get( )->flCollisionChangeOrigin = pEntity->GetCollisionChangeOrigin( );

		pEntity->GetAnimationLayers( pData.get( )->arrLayers );
		pEntity->GetPoseParameters( pData.get( )->flPoses );
		pEntity->GetBoneCache( pData.get( )->pMatricies[ VISUAL ] );

		arrBackupData[ i ].first = *pData.get( );
