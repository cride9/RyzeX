...
namespace ImGui
{
    ...
    IMGUI_API bool          Button(const char* label, const ImVec2& size = ImVec2(0, 0), bool = false);   // button
    ...
    IMGUI_API bool          ColorEdit4(const char* label, float col[4], bool bSameLine = false, ImGuiColorEditFlags flags = 0);
    ...
    IMGUI_API bool          ListBoxVector( const char* label, std::int32_t* currIndex, std::vector<std::string>& values, std::int32_t heightInItems = -1 ) noexcept;
...
struct ImFontConfig
{
    ...
    unsigned int    RasterizerFlags;        // 0        // Settings for custom font builder. THIS IS BUILDER IMPLEMENTATION DEPENDENT. Leave as zero if unsure.
