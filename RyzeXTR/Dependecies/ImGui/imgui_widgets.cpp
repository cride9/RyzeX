...
#include <string>
#include <format>
#include "../../Core/SDK/Menu/config.h"
...
{
    return ButtonEx(label, size_arg, ImGuiButtonFlags_None, bSelected, bFrame);
}
...
bool ImGui::Keybind(const char* str_id, int* current_key, int* key_style) {
    ...
    const ImVec2 label_size = CalcTextSize(KeyNames[*current_key]);
    ...
    SetNextWindowSize(it_anim->second.Maxs);
    ...
    {
        ImVec2 sizelmao = GetWindowSize();
        it_anim->second.Maxs = ImLerp(it_anim->second.Maxs, ImVec2(90, 75), g.IO.DeltaTime * 12.f);
        if (ImGui::Button("Hold", ImVec2(70, 15), cfg::m_iKeyStates[*current_key] == 1, false)) cfg::m_iKeyStates[*current_key] = 1;
        if (ImGui::Button("Toggle", ImVec2(70, 15), cfg::m_iKeyStates[*current_key] == 2, false)) cfg::m_iKeyStates[*current_key] = 2;
        if (ImGui::Button("Always-on", ImVec2(70, 15), cfg::m_iKeyStates[*current_key] == 3, false)) cfg::m_iKeyStates[*current_key] = 3;
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.BGOffset = ImLerp(it_anim->second.BGOffset, *v ? ImVec2(0, 0) : ImVec2(2, 2), g.IO.DeltaTime * ANIMATIONSPEED);
    it_anim->second.BG = ImLerp(it_anim->second.BG, *v ? clr::theme : clr::element_bg, g.IO.DeltaTime * ANIMATIONSPEED);
    it_anim->second.Frame = ImLerp(it_anim->second.Frame, *v ? clr::element_frame_active : clr::element_frame, g.IO.DeltaTime * ANIMATIONSPEED);
    it_anim->second.Mark = ImLerp(it_anim->second.Mark, *v ? ImColor(255, 255, 255) : ImColor(clr::element_bg), g.IO.DeltaTime * ANIMATIONSPEED);
    it_anim->second.MarkAnim = ImLerp(it_anim->second.MarkAnim, *v ? 0.f : square_sz, g.IO.DeltaTime * 3.f);
    it_anim->second.Text = ImLerp(it_anim->second.Text, *v ? clr::text_active : hovered ? clr::text_hov : clr::text_off, g.IO.DeltaTime * ANIMATIONSPEED);

    // frame
    window->DrawList->AddRect(check_bb.Min, check_bb.Max, GetColorU32(it_anim->second.Frame), style.FrameRounding);

    // fill
    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(it_anim->second.BGOffset), check_bb.Max - ImVec2(it_anim->second.BGOffset), GetColorU32(it_anim->second.BG), style.FrameRounding);

    // checkmark
    PushClipRect(check_bb.Min + ImVec2(0, 0), check_bb.Max - ImVec2(it_anim->second.MarkAnim, 0), true);
    RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(square_sz / 2, square_sz / 2) / 2, GetColorU32(it_anim->second.Mark), square_sz / 2);
    PopClipRect();

    // label
    // edit: cride -> Addtext renders the ##2 id aswell while rendertext is not so I changed it kekw
    ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y);
    PushStyleColor(ImGuiCol_Text, it_anim->second.Text);
    RenderText(label_pos, label);
    //window->DrawList->AddText(label_pos, GetColorU32(it_anim->second.Text), label);
    PopStyleColor();
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, float labelposx)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    {
        if (g.LogEnabled)
            LogSetNextTextDecoration("{", "}");
        RenderTextClipped(bb.Min + style.FramePadding + ImVec2(2.f, 0.f), ImVec2(value_x2, bb.Max.y) + ImVec2(2.f, 0.f), preview_value, NULL, NULL);
    }
    //SameLine(GetContentRegionAvail().x - label_size.x);
    //if (label_size.x > 0)
    //    RenderText(ImVec2(labelposx/* - label_size.x*/, bb.Min.y + style.FramePadding.y), label);
    SameLine(GetContentRegionAvail().x - label_size.x);
...
{
	static std::map<ImGuiID, ComboPopUpAnim> anim;
	auto it_anim = anim.find(popup_id);

	if (it_anim == anim.end())
	{
		anim.insert({ popup_id, ComboPopUpAnim() });
		it_anim = anim.find(popup_id);
	}
    ...
    if (!IsPopupOpen(popup_id, ImGuiPopupFlags_None))
    {
		...
		it_anim->second.sizeMax = ImVec2(0, 0);
        return false;
    }
    ...
    else
    {
		...
		it_anim->second.sizeMax = ImLerp(it_anim->second.sizeMax, constraint_max, g.IO.DeltaTime * 4.f);

        SetNextWindowSizeConstraints(constraint_min, it_anim->second.sizeMax);
    }
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ...
    float flMaxSize = GetContentRegionAvail().x;
    // The old Combo() API exposed "popup_max_height_in_items". The new more general BeginCombo() API doesn't have/need it, but we emulate it here.
    if (popup_max_height_in_items != -1 && !(g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSizeConstraint))
        SetNextWindowSizeConstraints(ImVec2(0, 0), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)));

    if (!BeginCombo(label, preview_value, ImGuiComboFlags_None, flMaxSize))
        return false;
...
bool ImGui::MultiComboBox(const char* label, const char* items[], bool* selectableItems, int size) {
	...
	if (ImGui::BeginCombo(label, preview != "" ? preview.c_str() : "None", ImGuiComboFlags_NoArrowButton)) {
		...
		for (size_t i = 0; i < size; i++) {
			...
			ImGui::Selectable(items[i], &selectableItems[i], ImGuiSelectableFlags_DontClosePopups);
		}
		ImGui::EndCombo();
	}
	return true;
...
struct SliderScalarAnim
{
    ImVec4 BG, Frame, Text, Grab;
	ImVec2 Pos;
    float Position;
};
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f) - ImVec2(20, 0));
    ...
    if (it_anim == anim.end())
    {
        anim.insert({ id, SliderScalarAnim() });
        it_anim = anim.find(id);
    }

    float flValue = (*(float*)p_data);
    int iValue = (*(int*)p_data);
    it_anim->second.BG = ImLerp(it_anim->second.BG, flValue != 0 || iValue != 0 ? clr::element_bg_active : clr::element_bg, g.IO.DeltaTime * ANIMATIONSPEED);
    it_anim->second.Frame = ImLerp(it_anim->second.Frame, flValue != 0 || iValue != 0 ? clr::element_frame_active : clr::element_frame, g.IO.DeltaTime * ANIMATIONSPEED);
    it_anim->second.Grab = ImLerp(it_anim->second.Grab, flValue != 0 || iValue != 0 ? clr::theme : clr::element_bg, g.IO.DeltaTime * ANIMATIONSPEED);
    it_anim->second.Text = ImLerp(it_anim->second.Text, flValue != 0 || iValue != 0 ? clr::text_active : hovered ? clr::text_hov : clr::text_off, g.IO.DeltaTime * ANIMATIONSPEED);

    // frame
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(it_anim->second.Frame), style.FrameRounding);

    // fill4
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(it_anim->second.BG), style.FrameRounding);
	...
	if (it_anim->second.Pos.x != GetWindowPos().x) {
		it_anim->second.Position = grab_bb.Max.x;
		it_anim->second.Pos = GetWindowPos();
	}

    // slider grab pos
    it_anim->second.Position = ImLerp(it_anim->second.Position, grab_bb.Max.x, g.IO.DeltaTime * 12);

    // clamp the slider grab position to the bounds of the slider
    it_anim->second.Position = ImClamp(it_anim->second.Position, frame_bb.Min.x + 2, frame_bb.Max.x);
    ...
    PushStyleColor(ImGuiCol_Text, it_anim->second.Text);
    {
        ...
        if ((std::strcmp(label, XorStr("Damage")) == 0 || std::strcmp(label, XorStr("Override##2")) == 0) && iValue > 100) {
            int leftNumber = iValue - 100;
            RenderTextClipped(frame_bb.Min, frame_bb.Max, std::format("HP+{}", leftNumber).c_str(), value_buf_end, NULL, ImVec2(0.5f, 0.5f));
        }
        else
            RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    ImVec2 pos = window->DC.CursorPos + ImVec2(5.f, 0.f);
    pos.y += window->DC.CurrLineTextBaseOffset;
    ItemSize(size, 0.0f);
	...
	static std::map<ImGuiID, SelectableAnim> anim;
	auto it_anim = anim.find(id);
	if (it_anim == anim.end()) {
		anim.insert({ id, SelectableAnim() });
		it_anim = anim.find(id);
	}
	...
	{
		// selectable lmao
		it_anim->second.BG = ImLerp(it_anim->second.BG, (held && hovered) ? clr::bg_selectableAct : hovered ? clr::bg_selectableHov : clr::bg_selectable, g.IO.DeltaTime * ANIMATIONSPEED);

		PushStyleColor(ImGuiCol_Header, it_anim->second.BG);
		//const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header);
		RenderFrame(bb.Min, bb.Max, GetColorU32(ImGuiCol_Header), false, 0.0f);
		PopStyleColor();
	}
	else
		it_anim->second.BG = ImVec4(0, 0, 0, 0);
	...
	it_anim->second.Text = ImLerp(it_anim->second.Text, (held && hovered) ? clr::text_selectable : hovered ? clr::text_selectable : clr::text_off, g.IO.DeltaTime * ANIMATIONSPEED);

	PushStyleColor(ImGuiCol_Text, selected ? clr::text_selectable : it_anim->second.Text);
...
bool ImGui::BeginListBox(const char* label, const ImVec2& size_arg)
{
    ...
    ImRect bb(frame_bb.Min - ImVec2(0.f, 5.f), frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    g.NextItemData.ClearFlags();
...
bool ImGui::ListBoxVector(const char* label, int* currIndex, std::vector<std::string>& values, int heightInItems) noexcept {
