...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, bool draw, ImVec4 color2)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
	...
	//if (draw) {
	//	RenderFrame(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y + 2), ImGuiCol_Text, false, style.FrameRounding);
	//}

    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(bb, id);
	if (draw)
		RenderFrame(ImVec2(bb.Min.x, bb.Max.y - 1), ImVec2(bb.Max.x, bb.Max.y + 2), col, false, 10.f);
...
bool ImGui::ButtonCenteredOnLine(const char* label, float alignment, bool selected)
{
	...
	return ImGui::Button(label, ImVec2(0,0), selected);
}

bool ImGui::Button(const char* label, const ImVec2& size_arg, bool selected)
{
    return ButtonEx(label, size_arg, ImGuiButtonFlags_None, selected);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    else if (*v)
    {
        ...
        const float pad = ImMax (1.0f, (float) (int) (check_sz));

        window-> DrawList-> AddRectFilled (check_bb.Min + ImVec2 (pad - 2, pad - 2), check_bb.Max-ImVec2 (pad - 2, pad - 2), GetColorU32 (ImGuiCol_CheckMark), style.FrameRounding);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    RenderTextClipped(text_min + ImVec2(5, 0), text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb);
